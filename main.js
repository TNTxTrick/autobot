    ...
    if (cUser) {
      ...
      if (existingUser) {
        ...
        return res.status(400).json({
          error: false,
          message: "Logging In Session Detected! You are currently logging in.",
          user: existingUser
        });
      } else {
        try {
          ...
          res.status(200).json({
            success: true,
            message: 'Logging In Session Successfully! You are now logging in.'
          });
        } catch (error) {
          console.error(error);
          res.status(400).json({
...
app.listen(20325, () => {
  console.log(`Server is running at http://localhost:20325`);
});
  ...
  return new Promise((resolve, reject) => {
    login({
      ...
      try {
        ...
        if (!userInfo || !userInfo[userid]?.name || !userInfo[userid]?.profileUrl || !userInfo[userid]?.thumbSrc) throw new Error('Checkpoint Detected! Please check your account.');
        const {
          name,
          profileUrl,
          thumbSrc
        } = userInfo[userid];
        let time = (JSON.parse(fs.readFileSync('./data/history/history.json', 'utf-8')).find(user => user.userid === userid) || {}).time || 0;
      ...
      try {
        api.listenMqtt(async (error, event) => {
          ...
          let history = await fs.existsSync('./data/history/history.json') ? JSON.parse(fs.readFileSync('./data/history/history.json')) : {};
          ...
          if (hasPrefix && aliases(command)?.hasPrefix === false) {
            api.sendMessage(`[ ! ] This ccommand is not require a prefix to starts with.`, event.threadID, event.messageID);
            return;
          }
          ...
          if (event.body && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && aliases(command)?.name && enableCommands[0].commands.includes(aliases(command?.toLowerCase())?.name)) {
            if (blacklist.includes(event.senderID)) {
              api.sendMessage('You have been banned!', event.threadID, event.messageID);
              return;
            }
          }
          if (event.body && aliases(command)?.name) {
            ...
            const delay = aliases(command)?.cooldowns ?? 0;
          ...
          if (event.body && !command && event.body?.toLowerCase().startsWith(prefix.toLowerCase())) {
            api.sendMessage(`Invalid command! \n\nUse ${prefix}help to see the list of available commands.`, event.threadID, event.messageID);
            return;
          }
          if (event.body && command && prefix && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && !aliases(command)?.name) {
            api.sendMessage(`Invalid command '${command}'. \n\nUse ${prefix}help to see the list of available commands.`, event.threadID, event.messageID);
            return;
          }
          for (const {
              handleEvent,
...
async function deleteThisUser(userid) {
  const configFile = './data/history/history.json';
...
async function main() {
  ...
  cron.schedule(`*/${adminOfConfig[0].masterKey.autoRestart} * * * *`, async () => {
    const history = JSON.parse(fs.readFileSync('./data/history/history.json', 'utf-8'));
    ...
    await fs.writeFileSync('./data/history/history.json', JSON.stringify(history, null, 2));
    process.exit(1);
  });
  try {
